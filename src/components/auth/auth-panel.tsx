...
export default function AuthPanel({ auth, variant, canClose = false, onClose }: AuthPanelProps) {
  ...
  return (
    <article
      className={isModal ? 'td-card w-full max-w-xl bg-td-bg shadow-card' : 'rounded-[14px] border border-td-line bg-td-surface'}
      aria-label={isModal ? 'auth-modal' : 'settings-auth-panel'}
    >
      <header className={isModal ? 'border-b border-td-line px-4 py-3 sm:px-5' : 'border-b border-td-line px-4 py-3'}>
        <div className="flex items-start gap-2">
          <div className="min-w-0 flex-1">
            ...
            <h3 className="text-xl text-td-text">{stageCopy.title}</h3>
      ...
      <section className={isModal ? 'space-y-4 px-4 py-4 sm:px-5 sm:py-5' : 'space-y-4 px-4 py-4'}>
        ...
        {state.stage === 'ready' ? (
          <div className="text-sm text-td-muted">
            <p>主密码 7 天内免输已生效。</p>
          </div>
        ) : null}
      </section>
