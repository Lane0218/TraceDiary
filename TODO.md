...
- 更新时间：`2026-02-11`
- 总任务：`80`
- 状态统计：`DONE=65` / `DOING=1` / `TODO=14` / `BLOCKED=0`
- 当前进行中：`TD-FUNC-009`
...
| `TD-FUNC-009` | `DOING` | 修复基础 Markdown 标题/列表视觉差异并新增可编辑源码模式 | 标题（`#`）在可视化模式具备明显层级样式；无序/有序列表样式可见；支持“可视化/源码”双模式切换且内容双向同步、不丢失 | `src/components/editor/markdown-editor.tsx` `src/components/editor/markdown-editor.css` `src/__tests__/integration/markdown-editor.integration.test.tsx` `e2e/specs/daily-edit.spec.ts` `e2e/specs/yearly-summary.spec.ts` `TODO.md` | 进行中 | — |
