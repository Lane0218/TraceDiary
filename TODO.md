...
- 更新时间：`2026-02-09`
- 总任务：`54`
- 状态统计：`DONE=39` / `DOING=1` / `TODO=15` / `BLOCKED=0`
- 当前进行中：`TD-SYNC-009`
...
| `TD-SYNC-009` | `DOING` | 修复手动上传在并发场景下静默失败（禁止并发并提供明确反馈） | 上传进行中点击手动按钮会给出“请稍候”提示且不静默；按钮在同步中禁用；失败分支均有可消费错误信息 | `src/hooks/use-sync.ts` `src/pages/workspace.tsx` `src/pages/yearly-summary.tsx` `src/hooks/__tests__/use-sync.test.ts` `src/__tests__/integration/editor.integration.test.tsx` | — | — |
