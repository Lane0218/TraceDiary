...
- 搜索入口仅在日记页提供，不在年度总结页提供
- 仅检索 `type = "daily"` 的日记条目
- 仅支持文字字面量匹配，不支持正则表达式
- 数据范围仅限当前设备本地已缓存内容（IndexedDB），不触发自动全量远端拉取

#### 4.9.2 查询规则

1. 用户输入关键词后，系统对关键词做 `trim` 处理
2. 若关键词为空字符串，不执行检索并展示搜索空态
3. 按 Markdown 原文执行子串包含匹配（`includes` 语义）
4. 不做分词、不做同义词扩展、不做正则解析
5. 默认按原文逐字符匹配（区分大小写）

#### 4.9.3 结果展示与交互

- 左侧面板新增“搜索”标签页，与“往年今日 / 统计”并列
- 搜索结果使用虚拟滚动列表，避免命中数量过多导致面板溢出
- 单条结果固定高度，展示内容为：
  - 日期（`YYYY-MM-DD`）
  - 命中片段（单行展示，超出省略）
- 命中片段使用“首个命中位置”前后固定窗口截取；单行展示下需优先保证首个命中关键词可见
- 点击结果项后，跳转并切换到对应日期日记

#### 4.9.4 性能与上限策略

- 输入后采用 200ms 防抖，减少频繁重算
- 结果按日期倒序（最新优先）
- 结果最大展示数量为 200 条，超出时提示“仅展示前 200 条”
- 单篇日记仅返回首个命中片段，避免列表膨胀

#### 4.9.5 状态反馈

- 搜索中：显示“正在搜索...”
- 无结果：显示“未找到包含‘关键词’的日记”
- 本地数据读取异常：复用日记页现有错误态，不新增独立异常体系
...
```typescript
interface DiarySearchQuery {
  keyword: string;                // 用户输入关键词（trim 前原始值）
  normalizedKeyword: string;      // trim 后关键词
  source: 'local_daily_only';     // 仅本地 daily 范围
  limit: number;                  // 默认 200
}

interface DiarySearchResult {
  entryId: `daily:${string}`;
  date: string;                   // YYYY-MM-DD
  snippet: string;                // 命中片段（单行展示，超出由 UI 省略）
  matchIndex: number;             // 首个命中位置
  contentLength: number;          // 原文长度
}

interface DiarySearchResponse {
  query: DiarySearchQuery;
  totalMatched: number;           // 命中总数（截断前）
  returnedCount: number;          // 返回条数（<= limit）
...
- 搜索仅基于 `DiaryEntry.type = 'daily'` 且内容非空条目
- 匹配目标为 `content` 原文（Markdown 文本）
- 当前版本不新增 IndexedDB 持久化索引与 schema 迁移
...
展示搜索空态（不执行检索）
    ↓ 否
在本地缓存 daily 日记中执行子串匹配
    ↓
按日期倒序生成结果（最多 200 条）
    ↓
左侧虚拟列表显示“日期 + 单行片段（关键词高亮优先可见）”
    ↓
用户点击某条结果
    ↓
跳转并切换到该日期日记内容
...
- [ ] 编辑后不会自动上传，只有手动点击“push”才会 Push
- [ ] 手动 Pull 可拉取远端最新数据并更新本地
- [ ] 多设备数据能正确同步
- [ ] 冲突时基于 `sha mismatch` 弹出对话框让用户选择
- [ ] 网络断开时提示用户，恢复后由用户手动重试
...
- [ ] 搜索入口仅在日记页可见，年度总结页无搜索入口
- [ ] 仅检索 `daily` 日记，不检索 `yearly_summary`
- [ ] 关键词匹配为文字子串匹配，不支持正则表达式
- [ ] 检索范围仅限本地已缓存日记，不自动拉取远端全量
- [ ] 结果列表支持虚拟滚动，单条展示“日期 + 单行命中片段”
- [ ] 长文本命中时，关键词高亮不可落在可视省略号之后（关键词在结果行内可见）
- [ ] 命中数量过多时可滚动查看，且最多展示前 200 条
- [ ] 点击结果可跳转到对应日期并打开该日记内容
...
- [ ] 切换日期响应 ≤ 200ms
- [ ] 往年今日查询 ≤ 1 秒（5 年数据）
- [ ] 日记页搜索在 5000 条本地日记场景下单次检索耗时 ≤ 150ms（中端桌面设备）
- [ ] 编辑器输入延迟 ≤ 50ms
