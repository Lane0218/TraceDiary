...
3. 完成代码修改后，必须按第 8 节“测试执行策略（默认分层）”执行对应测试集合。
...
8. 向用户汇报：改动摘要、测试结果（含是否执行全量 E2E）、commit hash、push 结果。
...
- `DONE`：已完成、满足对应测试门禁并已 commit+push
- `BLOCKED`：受阻（需标注阻塞原因）
...
- 默认执行“分层必要测试”：`npm run lint` + `npm run test:unit` + `npm run test:integration` + 与改动直接相关的 E2E（`test:e2e:fast`、`test:e2e:changed`、`--grep` 或定向 spec）。
- 以下场景必须执行全量 E2E（`npm run test:e2e:full` 或等价全量命令）：
  - 合并到主分支前。
  - 发布前（含预发布与正式发布）。
...
- 若用户在当前任务中明确授权“进一步缩小测试范围”，可在默认分层基础上继续裁剪 E2E 范围。
- 裁剪后最小测试集合仍不得低于：`npm run lint` + `npm run test:unit` + `npm run test:integration` + 与改动直接相关的目标 E2E 用例。
- “直接相关”判定原则：仅覆盖本次改动触达的主链路与高风险分支（如同步冲突、认证、加解密、上传链路），避免无关回归。
- 任务汇报与 `TODO.md` 的 `测试记录` 必须明确写出：已执行测试、未执行项、用户授权依据。
...
- 本地分层命令以 `package.json` 为准，推荐使用：
  - `npm run test:e2e:fast`：`@smoke` 快速回归。
  - `npm run test:e2e:changed`：按 Git 变更范围执行。
  - `npm run test:e2e:last-failed`：仅重跑上次失败集。
  - `npm run test:e2e:slow`：执行 `@slow` 长耗时场景。
  - `npm run test:e2e:remote`：执行 `@remote` 远端依赖场景。
  - `npm run test:e2e:full`：全量 E2E。
  - `npm run test:e2e:local-shard:1` 与 `npm run test:e2e:local-shard:2`：本地分片并行。
...
- AI 在任务开工说明中必须声明：风险分级、选用测试命令、是否计划执行全量 E2E。
- `TODO.md` 的 `测试记录` 必须记录：实际命令、通过/失败摘要、未执行项及原因。
- 若已满足全量门禁条件，不得以“耗时长”为由跳过全量 E2E；必须执行或标记 `BLOCKED` 并写明阻塞原因。
